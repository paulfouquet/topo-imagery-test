...
from scripts.gdal.gdal_preset import DataType, get_gdal_command
...
def standardising(
    ...
    output_file_name = get_file_name_from_path(file)
    standardized_file_path = os.path.join(target_output, output_file_name)
    tiff = FileTiff(file)

    if not exists(standardized_file_path):
        with tempfile.TemporaryDirectory() as tmp_path:
            standardized_working_path = os.path.join(tmp_path, output_file_name)
            input_file = file
            ...
            command = get_gdal_command(data_type=DataType.ELEVATION, preset="lzw", epsg="2193")
...
def main() -> None:
    ...
    run_standardising(source, concurrency)
